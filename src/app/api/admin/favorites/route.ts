...
export async function GET() {
  try {
    // Общее количество избранных записей
    const totalRes = await pool.query('SELECT COUNT(*) FROM favorites')
    
    // Количество уникальных пользователей с избранным
    ...
    const topItemsRes = await pool.query(
      `SELECT f.clothing_item_id, COUNT(*) as count, c.title, c.image_url
       FROM favorites f
       LEFT JOIN clothing_items c ON f.clothing_item_id = c.id
       GROUP BY f.clothing_item_id, c.title, c.image_url
       ORDER BY count DESC
       LIMIT 5`
    )
    ...
    console.log('Favorites stats calculated:', {
      total: totalRes.rows[0].count,
      users: usersRes.rows[0].count,
      topItems: topItemsRes.rows.length,
      topUsers: topUsersRes.rows.length
    })
