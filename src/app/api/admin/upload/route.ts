...
export async function POST(req: NextRequest) {
  try {
    ...
    if (base64Image.length > 10 * 1024 * 1024) {
      return NextResponse.json({ error: 'Изображение слишком большое после конвертации' }, { status: 400 })
    }

    console.log('✅ Файл загружен в base64:', file.name, 'размер:', Math.round(buffer.length / 1024), 'KB')
